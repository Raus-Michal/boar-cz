...
class Kalendar {
 ...
 kryt_id="krit_dny"; // id DIV krytu, překrývající možnozst klikat do kalendáře
 poloha=0; // proměnná určuje polohu kalendáře 0===default, 1-krok o měsíc dále , 2-krok o dva měsíce dále
 book_den=[0,0,0]; // zápis booklého dne uživatelem : rok, měsíc, den
 facke_checked_id="fake-checked"; // id input type chacked - fake chacked, který nedošle formulář, dokud není od uživatele označený konkrétní den
 color_oznacen="rgb(87,168,110)"; // barva označeného buttonu s dnem v měsíci zvoleným uživatelem
 color_NEoznacen="white"; // barva neoznačeného buttonu s dny v měsíci
 z_posun_id="pb"; // začátek id vyplňovacích bloků pro posun buttonu s čísly dnů v kalendáři (id="pb1" až id="pb6")
block_days=[]; // pole s blokovanými datumy pro booking dne
load_data_book_block_day=false; // proměnná na true ukazuje, že data o dnech, které se mají blokovat, jsou z JSON souboru načtena a false, že nejsou načtena
load_data_time_out=null; // časovač pro REKLUZI funkce data_book_block_day()
error_load_element=["errror_load","flex","retry_load","errror_load_offline"]; // pole zahrnuje prvky,pro zobrazení errro okna - Potřebná data, nebyla ze serveru načtena. [0=id div, 1=display div, 2=id button Opakovat načtení, 3= element p v tomto okně upozorňující, že byl uživatel offline]
...
class Mesic_a_rok{
 ...
 id_kalendar="plocha_kalendar"; // ide Fielsetu kalendáře, kde jsou dny Po-Pá, dny v měsící 1-31, výplně 1-6
 touchStartX=0; // zachycení začátku pohybu uživatele prstem na obrazovce (osa X)
 touchEndX=0; // zachycení konce pohybu uživatele prstem na obrazovce (osa X)
 touchStartY=0; // zachycení začátku pohybu uživatele prstem na obrazovce (osa Y)
 touchEndY=0; // zachycení konce pohybu uživatele prstem na obrazovce (osa Y)
aktivace(){
...
// objekt zajišťuje potřebné funkcionality pro volbu času rezervace
 id_con="con_cas"; // ID hlavního kontejneru s časy rezervace
 id_logo=["app_logo_box","app_logo_svg","app_logo_text"]; // id k SVG logu [0=ID boxu v kterém je vloženo SVG logo Boar-cz, 1=ID samotného HTML SVG v kterém je logo, 2=ID contajneru textu pod logem]
 id_radio="cas"; // počátek ID input type radio cas1-cas14
 id_li="lic"; // počátek ID li v kterém je input type radio lic1-lic14
 class_nam=["zobraz_objekt","schovej_objekt"]; // názvy CSS tříd, které jsou používány pro animavce
 vybrany_cas=0; // vybraný čas uživatelem, kde 0 znamená, že čas nebyl vybrán a 1 je první čas
 casy=["9:00-9:30 hod.","9:30-10:00 hod.","10:00-10:30 hod.","10:30-11:00 hod.","11:00-11:30 hod.","11:30-12:00 hod.","12:00-12:30 hod.","12:30-13:00 hod.","13:00-13:30 hod.","13:30-14:00 hod.","14:00-14:30 hod.","14:30-15:00 hod.","15:00-15:30 hod.","15:30-16:00 hod."]; // všechny časy zadané slovně
rezervace=[]; // pole s rokem, měsíce, dnem a časem(1-14) rezervace
load_data_rezervace=false; // proměnná, která ukazuje, zda byly ze souboru JSON načteny rezervace, pokud ano===true, pokud ne===false
load_rezervace_time_out=null; // časovač pro REKLUZI funkce data_rezervace_blok_time()
...
boundOffs={}; // objekt, který zajistí správný způsob je skutečně uložit bindovanou funkci do proměnné
...
casovac_animace=null; // časovač animace pro její opakování
startTime=null; // proměnná hlídá kolik času animace uběhlo
...
// class bude zajišťovat hlavní chod celé aplikace rezervace
 id_boss_con="boss_con"; // id hlavního kontejneru aplikace
 id_form=["rezervace_form","dokoncit_form"]; // id formulářů
 id_button=["zmenit","ukaz_zasady","butt_zavinac"]; // id hlavních buttonů formulářů
 id_inputHost=["jmeno","email","phone","predmet"]; // id input, které vyplňoval návštěvník [návštěvník: jméno a příjmení, návštěvník: email, návštěvník: telefon, návštěvník: O čem bude hovor]
 id_checked="checken_souhlas"; // id checked Souhlasím se zpracováním osobních údajů
 id_predvolba_phone="predvolba"; // id input s predvolbou telefonní +420
 id_cas="slovne_cas_rezervace"; // id SPAN ve formuláři Dokončit rezervaci, kde se zapisuje čas rezervace
 id_den="slovne_den_rezervace"; // id SPAN ve formuláři Dokončit rezervaci, kde se zapisuje den rezervace Pondělí-Neděle
 id_den_v_mesici="ciselne_den_v_mesici_rezervace"; // id SPAN ve formuláři Dokončit rezervaci, kde se zapisuje den v měsíci rezervace 1-31
 id_mesic="slone_mesic_rezervace"; // id SPAN ve formuláři Dokončit rezervaci, kde se zapisuje měsíc rezervace leden-prosinec
 id_rok="ciselne_rok_rezervace"; // id SPAN ve formuláři Dokončit rezervaci, kde se zapisuje rok rezervace např. 2024
 id_token="token"; // id HTML input s tokenem
 slovne_datum=""; // v proměnné je slovně uložené celé datum rezervace
 slovne_cas=""; // v proměnné je uloženo slovně konkrétní čas rezervace
 dny=["Neděle","Pondělí","Úterý","Středa","Čtvrtek","Pátek","Sobota"]; // dny v týdnu
 mesice=["ledna","února","března","dubna","květena","června","července","srpna","září","října","listopadu","prosince"]; // měsíce v roce
...
zaznam_encrypted_token=""; // proměnná do sebe vloží zakódovaný token záznamu, který má být zrušen, pokud existuje
den_a_cas_rezervace=""; // proměnná do sebe vloží informaci o dnu a času rezervace, toto bude následně zasláno emailem tomu kdo rezervaci zrušil a mně, text bude vypadat např.: Čtvrtek, 20. února 2025, 13:30-14:00 hod.
 id_form="form_zruseni_rezervace"; // id formuláře v dialogovém okně: Zrušit rezervaci?
 id_butt="butt_zavinac2"; // id buttonů v dialogovém okně: Zrušit rezervaci? button @ a button Zrušit rezervaci
 id_input=["email2","duvod"]; // id input Email a Důvod zrušení v dialogovém okně: Zrušit rezervaci?
 id_span_cas_r=["slovne_den_rezervace2","ciselne_den_v_mesici_rezervace2","slone_mesic_rezervace2","ciselne_rok_rezervace2","slovne_cas_rezervace2"]; // id SPAN HTML elementů kde se uvádí termín rezervace, která má být zrušena např: Úterý, 24. prosince 2024, 15:00-15:30 hod.
delka_tokenu=32; // délka tokenu, počet zbaků, který má token
...
vytvor_zadost_na_zruseni(rok,mesic,den,cislo_casu) // (mesic 0-11, den 1-31 , cislo_casu 1-14)
{
