...
class Kalendar {
    constructor() {
        this.p_id = "b"; // počáteční id každého buttonu ke kterému je přiřazeno v HTML číslo 1-31 ... b1.b2->b31
        this.m_a_r_id = "mesic_a_rok"; // id inputu s měsícem a rokem
        this.den_id = "d"; // počáteční id každého dne v týdnu v kalendáři <p> Po,Út,St,Čt,Pá,So,Ne
        this.kryt_id = "krit_dny"; // id DIV krytu, překrývající možnozst klikat do kalendáře
        this.poloha = 0; // proměnná určuje polohu kalendáře 0===default, 1-krok o měsíc dále , 2-krok o dva měsíce dále
        this.book_den = [0, 0, 0]; // zápis booklého dne uživatelem : rok, měsíc, den
        this.facke_checked_id = "fake-checked"; // id input type chacked - fake chacked, který nedošle formulář, dokud není od uživatele označený konkrétní den
        this.color_oznacen = "rgb(87,168,110)"; // barva označeného buttonu s dnem v měsíci zvoleným uživatelem
        this.color_NEoznacen = "white"; // barva neoznačeného buttonu s dny v měsíci
        this.z_posun_id = "pb"; // začátek id vyplňovacích bloků pro posun buttonu s čísly dnů v kalendáři (id="pb1" až id="pb6")
        this.block_days = []; // pole s blokovanými datumy pro booking dne
        this.load_data_book_block_day = false; // proměnná na true ukazuje, že data o dnech, které se mají blokovat, jsou z JSON souboru načtena a false, že nejsou načtena
        this.load_data_time_out = null; // časovač pro REKLUZI funkce data_book_block_day()
        this.error_load_element = ["errror_load", "flex", "retry_load", "errror_load_offline"]; // pole zahrnuje prvky,pro zobrazení errro okna - Potřebná data, nebyla ze serveru načtena. [0=id div, 1=display div, 2=id button Opakovat načtení, 3= element p v tomto okně upozorňující, že byl uživatel offline]
    }
    ...
    restart_dnu_v_kalendari() {
        ...
        for (let i = 1; i < 32; i++) {
            ...
            if (button_i) {
                ...
                button_i.disabled = false; // Odblokuje všechny buttony
            }
        }
    }
    ;
    ...
    book_block_day() {
        ...
        const block_days = this.block_days; // převede globýlní pole s blokovanými dny do lokální proměnné
        ...
        let dny_k_blokaci = []; // pole, kde se budou zapisovat dny k blokaci v měsíci
        for (let i = 0; i < d; i++) {
        ...
        setTimeout(async () => {
            ...
            if (jsonData) {
                jsonData.data.forEach((item) => {
                    const dateArray = [item.rok, item.mesic, item.den];
    ...
    error_load_data() {
        ...
        this.dny_klikat(true); // metoda zamezuje anebo umožňuje uživateli klikat na dny 1-31, pokud je do funkce zasláno TRUE=klikání, je umožněno; pokud FALSE=klikání je zamezeno
        const kontajner = document.getElementById(this.error_load_element[0]); // kontajner s informací: Potřebná data, nebyla ze serveru načtena.
        const k_button = document.getElementById(this.error_load_element[2]); // buton kontajneru: Opakovat načtení
        const p_offline = document.getElementById(this.error_load_element[3]); // P element s informací: Byl jste offline
        if (kontajner) {
            // pokud html Element existuje
    ...
    ;
    error_load_data_close(load = false) {
    ...
    handleEvent(e) {
        ...
        const k = e.target.closest("button").id; // načte id buttonu na který bylo kliknuto
        ...
        const cislo_dne = parseInt(`${k[1]}${k[2]}`); // z id buttonu odstraní pomocí parseTnt veškerý text a získá číslo dne jako integer
        let a_m = datum.mesic_v_roce + this.poloha; // datum.mesic_v_roce je gettter, kde návratová hodnota je aktuální měsíc v roce, kde leden je 0 a prosinec 11 + this.poloha určuje aktuální polohu uživatele v kalendáři
        let a_r = datum.aktualni_rok; // datum.aktualni_rok je getter, kde návratová hodnota je aktuální rok: 2024,2025 ...
        if (a_m > 11) {
            // pokud bude měsíc prosinec, začnou měsíce od ledna a přičte se rok
...
class Mesic_a_rok {
    constructor() {
        this.id_posun = ["m_minus", "m_plus"]; // id šipek s posunem měsíce 0===vzad, 1===vpřed
        this.id_text = "mesic_a_rok"; // id inputu s textem měsíc a rok
        this.id_kalendar = "plocha_kalendar"; // ide Fielsetu kalendáře, kde jsou dny Po-Pá, dny v měsící 1-31, výplně 1-6
        this.touchStartX = 0; // zachycení začátku pohybu uživatele prstem na obrazovce (osa X)
        this.touchEndX = 0; // zachycení konce pohybu uživatele prstem na obrazovce (osa X)
        this.touchStartY = 0; // zachycení začátku pohybu uživatele prstem na obrazovce (osa Y)
        this.touchEndY = 0; // zachycení konce pohybu uživatele prstem na obrazovce (osa Y)
    }
    aktivace() {
        ...
        } // Pokud je event listener označen jako pasivní ({ passive: true }), znamená to, že prohlížeč ví, že event handler nebude volat preventDefault(). To umožňuje prohlížeči optimalizovat chování stránky, což může vést ke zvýšení výkonu, zejména při posouvání na dotykových zařízeních. Jinými slovy, pasivní event listener říká prohlížeči: "Nebudu měnit výchozí chování této události, můžeš ji tedy zpracovat okamžitě."
    }
    ;
    handleGesture(e) {
    ...
    ;
    handleEvent(e, pohyb = 0) {
        const k = e.currentTarget.id; // id odkazuje na prvek, na který je navázán posluchač události
        let problik_posunu = () => {
...
class Datum {
    constructor() {
        this.mesice = ["leden", "únor", "březen", "duben", "květen", "červen", "červenec", "srpen", "září", "říjen", "listopad", "prosinec"]; // měsíce v roce
    }
    get den_v_tydnu() {
...
class Cas_rezervace {
    constructor() {
        ...
        this.id_con = "con_cas"; // ID hlavního kontejneru s časy rezervace
        this.id_logo = ["app_logo_box", "app_logo_svg", "app_logo_text"]; // id k SVG logu [0=ID boxu v kterém je vloženo SVG logo Boar-cz, 1=ID samotného HTML SVG v kterém je logo, 2=ID contajneru textu pod logem]
        this.id_radio = "cas"; // počátek ID input type radio cas1-cas14
        this.id_li = "lic"; // počátek ID li v kterém je input type radio lic1-lic14
        this.class_nam = ["zobraz_objekt", "schovej_objekt"]; // názvy CSS tříd, které jsou používány pro animavce
        this.vybrany_cas = 0; // vybraný čas uživatelem, kde 0 znamená, že čas nebyl vybrán a 1 je první čas
        this.casy = ["9:00-9:30 hod.", "9:30-10:00 hod.", "10:00-10:30 hod.", "10:30-11:00 hod.", "11:00-11:30 hod.", "11:30-12:00 hod.", "12:00-12:30 hod.", "12:30-13:00 hod.", "13:00-13:30 hod.", "13:30-14:00 hod.", "14:00-14:30 hod.", "14:30-15:00 hod.", "15:00-15:30 hod.", "15:30-16:00 hod."]; // všechny časy zadané slovně
        this.rezervace = []; // pole s rokem, měsíce, dnem a časem(1-14) rezervace
        this.load_data_rezervace = false; // proměnná, která ukazuje, zda byly ze souboru JSON načteny rezervace, pokud ano===true, pokud ne===false
        this.load_rezervace_time_out = null; // časovač pro REKLUZI funkce data_rezervace_blok_time()
    }
    ...
    data_rezervace_blok_time() {
        ...
        const rezervace = this.rezervace; // převede globýlní pole s rezervací (rok,měsíc 0-11,den,čas 1-14) do lokální proměnné
        const book_den = kalendar.rezervovane_datum; // getter vrací aktuální rezervované datum uživatelem [rok, měsíc , den]
        const cas_shody = []; // do pole budou zapsány všechny časy (1-14) odpovídající stejnému roku, měsíci a dnu rezervace
    ...
    load_rezervace() {
        ...
        setTimeout(async () => {
            ...
            if (jsonData) {
                jsonData.data.forEach((item) => {
                    const dateArray = [item.rok, item.mesic, item.den, item.cas_rezervace];
    ...
    ;
    handleEvent(e) {
        const k = e.target.id; // zjistí ID prvku na který byl klik proveden
        const number = parseInt(k.replace(/\D/g, '')); // .replace(/\D/g, '') odstraní všechny nečíselné znaky (což jsou ty, které nejsou číslice) z řetězce a parseInt() převádí tento řetězec na celé číslo.
    ...
    zobrazit_casy() {
        ...
        const logo_box = document.getElementById(this.id_logo[0]); // kontejner s logem Boar-cz
        const logo_svg = document.getElementById(this.id_logo[1]); // SVG kontajner pro samotné SVG logo
        const logo_text = document.getElementById(this.id_logo[2]); // SVG kontajner pro samotné SVG logo
...
class Dia {
    constructor() {
        ...
        this.boundOffs = {}; // objekt, který zajistí správný způsob je skutečně uložit bindovanou funkci do proměnné
        this.open_dialog = false; // proměnná, která sleduje jestli je otevřen dialogové okno s dotazem na zrušení rezervace, pokud ANO===TRUE, pokud NE===false
        this.casovac_animace = null; // časovač animace pro její opakování
        this.startTime = null; // proměnná hlídá kolik času animace uběhlo
        this.dia_zasady = {
    ...
    get addDia_zasady() {
        ...
        return Object.values(this.dia_zasady); // vrátí všechny hodnoty objektu jednu po druhé, jejich použití je možné pomocí speed operátoru ...
    }
    ;
    on(id_dialog, id_button_z = "", id_kotva_top = "", id_button_scroll = "", id_kotva_bottom = "") {
    ...
    ;
    off(id_dialog, id_button_z = "", id_button_scroll = "") {
    ...
    wait_activ() {
        ...
        const id_an = (_a = this.dia_waiting.id_animace) !== null && _a !== void 0 ? _a : "cir_1"; // operátor nulového slučování (nullish coalescing operator), který poskytne výchozí hodnotu, pokud je hodnota undefined nebo null
...
class Boss {
    constructor() {
        ...
        this.id_boss_con = "boss_con"; // id hlavního kontejneru aplikace
        this.id_form = ["rezervace_form", "dokoncit_form"]; // id formulářů
        this.id_button = ["zmenit", "ukaz_zasady", "butt_zavinac"]; // id hlavních buttonů formulářů
        this.id_inputHost = ["jmeno", "email", "phone", "predmet"]; // id input, které vyplňoval návštěvník [návštěvník: jméno a příjmení, návštěvník: email, návštěvník: telefon, návštěvník: O čem bude hovor]
        this.id_checked = "checken_souhlas"; // id checked Souhlasím se zpracováním osobních údajů
        this.id_predvolba_phone = "predvolba"; // id input s predvolbou telefonní +420
        this.id_cas = "slovne_cas_rezervace"; // id SPAN ve formuláři Dokončit rezervaci, kde se zapisuje čas rezervace
        this.id_den = "slovne_den_rezervace"; // id SPAN ve formuláři Dokončit rezervaci, kde se zapisuje den rezervace Pondělí-Neděle
        this.id_den_v_mesici = "ciselne_den_v_mesici_rezervace"; // id SPAN ve formuláři Dokončit rezervaci, kde se zapisuje den v měsíci rezervace 1-31
        this.id_mesic = "slone_mesic_rezervace"; // id SPAN ve formuláři Dokončit rezervaci, kde se zapisuje měsíc rezervace leden-prosinec
        this.id_rok = "ciselne_rok_rezervace"; // id SPAN ve formuláři Dokončit rezervaci, kde se zapisuje rok rezervace např. 2024
        this.id_token = "token"; // id HTML input s tokenem
        this.slovne_datum = ""; // v proměnné je slovně uložené celé datum rezervace
        this.slovne_cas = ""; // v proměnné je uloženo slovně konkrétní čas rezervace
        this.dny = ["Neděle", "Pondělí", "Úterý", "Středa", "Čtvrtek", "Pátek", "Sobota"]; // dny v týdnu
        this.mesice = ["ledna", "února", "března", "dubna", "květena", "června", "července", "srpna", "září", "října", "listopadu", "prosince"]; // měsíce v roce
    }
    rovna_vyska_form() {
        // metoda zajistí že oba formuláře rezrvace budou mít stejnou výšku, budou oba vysoké jako ten nejvyšší
        const form1 = document.getElementById(this.id_form[0]); // form 1
        const form2 = document.getElementById(this.id_form[1]); // form 2
        if (form1 && form2) {
            let v1 = 0;
            let v2 = 0;
    ...
    reset_aplikace(jak = "") {
        ...
        if (jak != "castecne") {
            ...
            for (let i = 0; i < d; i++) {
                ...
                const input = document.getElementById(id_inputy[i]); // načte HTML input jeden po druhém
                if (input) {
                    // pokud HTML element existuje
        ...
        const fake_checked = document.getElementById(kalendar.facke_checked_id); // checked fake, krerý slouží jen pro upozornění, aby uživatel zatrhl den 1-31, pokud tak neučinil
        if (fake_checked) {
            // pokud HTML element existuje
...
input_predvolba.addEventListener("focus", (event) => {
  ...
  setTimeout(() => {
    ...
    zobrazeni_datumu() {
        ...
        const dny = this.dny; // dny v týdnu
        const mesice = this.mesice; // měsíce v roce
        const den_rezervace_uzivatel = kalendar.rezervovane_datum; // getter vrátí datum zadané uživatelem [rok, měsíc(0-11), den]:number[]
        ...
        const den_v_tydnu_slovne = dny[den_v_tydnu]; // konkrétní den v roce zadaný uivatelem slovně Neděle až Pátek
        const den_v_mesici = den_rezervace_uzivatel[2].toString(); // den v měsíci převeden na string
        const mesic_v_roce_slovne = mesice[den_rezervace_uzivatel[1]]; // měsíc v roce slovně
        const rok = den_rezervace_uzivatel[0].toString(); // rok převeden na string
    ...
    handleEvent(e) {
        ...
        if (e.target.closest("button") && e.target.closest("button").id) {
            ...
            const k_b = e.target.closest("button").id; // načte id buttonu na který bylo kliknuto
    ...
    async rezervovat() {
        ...
        const den_rezervace_uzivatel = kalendar.rezervovane_datum; // getter vrátí datum zadané uživatelem [rok, měsíc(0-11), den]:number[]
        const cas_rezervace_uzivatel = cas_rezervace.cislo_vybraneho_casu; // getter vrací číslo vybraného času uživatelem 1-14
        const input_hidden = document.getElementById(this.id_token);
        ...
        const datum_rezervace_slovne = this.slovne_datum; // slovně zapsaný celé datum rezervace pro zaslání emalem (např.: Úterý, 1. dubna 2025)
        data_pro_Email[5] = datum_rezervace_slovne;
        const cas_rezervace_slovne = this.slovne_cas; // slovně zapsaný čas rezervace pro rozesílání emailem (např.:9:00-9:30 hod.)
        data_pro_Email[6] = cas_rezervace_slovne;
        const currentUrl = window.location.href; // Tento kód uloží aktuální URL do proměnné
        const cleanUrl = currentUrl.split("#")[0].split("?")[0]; // očistí url adresu, kdyby obsahovala hash # anebo honotu za ?
        ...
        const sendRequest = async () => {
            try {
                ...
                setTimeout(async () => {
                    ...
                    try {
                        const response = await fetch("config/distributor-rezervace.php", {
                            method: "POST",
                            headers: {
...
class Zrusit_rezervaci {
    constructor() {
        ...
        this.zaznam_encrypted_token = ""; // proměnná do sebe vloží zakódovaný token záznamu, který má být zrušen, pokud existuje
        this.den_a_cas_rezervace = ""; // proměnná do sebe vloží informaci o dnu a času rezervace, toto bude následně zasláno emailem tomu kdo rezervaci zrušil a mně, text bude vypadat např.: Čtvrtek, 20. února 2025, 13:30-14:00 hod.
        this.id_form = "form_zruseni_rezervace"; // id formuláře v dialogovém okně: Zrušit rezervaci?
        this.id_butt = "butt_zavinac2"; // id buttonů v dialogovém okně: Zrušit rezervaci? button @ a button Zrušit rezervaci
        this.id_input = ["email2", "duvod"]; // id input Email a Důvod zrušení v dialogovém okně: Zrušit rezervaci?
        this.id_span_cas_r = ["slovne_den_rezervace2", "ciselne_den_v_mesici_rezervace2", "slone_mesic_rezervace2", "ciselne_rok_rezervace2", "slovne_cas_rezervace2"]; // id SPAN HTML elementů kde se uvádí termín rezervace, která má být zrušena např: Úterý, 24. prosince 2024, 15:00-15:30 hod.
        this.delka_tokenu = 32; // délka tokenu, počet zbaků, který má token
    }
    zaslat_zadost_na_zruseni() {
        // metoda zajistí pomocí fetch a PHP, vymazání rezervace z JSON souboru
        const input_hidden = document.getElementById(boss.id_token); // HTML input type hidden s tokenem
        let token = ""; // v proměnné bude uložen token, který bude zároveň heslem ke každé rezervaci - bude mít pokaždé stejnou délku 32 znaků!!!
        if (input_hidden) {
            token = input_hidden.value; // načte token z HTML elementu input type hidden
        }
        ...
        const inp_email = document.getElementById(this.id_input[0]); // HTML input s emailem
        ...
        const inp_duvod = document.getElementById(this.id_input[1]); // HTML input s důvodem zrušení
        ...
        const sendRequest = async () => {
            try {
                ...
                setTimeout(async () => {
                    // Odeslání požadavku na server
                    try {
                        const response = await fetch("config/zadost-zruseni-rezervace.php", {
                            method: "POST",
                            headers: {
                                "Content-Type": "application/x-www-form-urlencoded"
                            },
    ...
    handleEvent(e) {
        ...
        if (k === this.id_butt) {
            ...
            const input_email = document.getElementById(this.id_input[0]); // načte HTML input email v dialogovém okně: Zrušit rezervaci?
            if (input_email) {
    ...
    posluchace_activ() {
        ...
        const form = document.getElementById(this.id_form); // načte HTML objekt formuláře
        if (form) {
            // pokud HTML objekt existuje
            form.addEventListener("submit", this); // Zabrání výchozímu chování (odeslání formuláře)
        }
        const butt = document.getElementById(this.id_butt); // načte HTML element buttonu @
        if (butt) {
            // pokud HTML element existuje
    ...
    ;
    vytvor_zadost_na_zruseni(rok, mesic, den, cislo_casu) {
        ...
        const s_denS = document.getElementById(this.id_span_cas_r[0]); // span, kde má být zapsán den rezervace slovně
        const s_denC = document.getElementById(this.id_span_cas_r[1]); // span, kde má být zapsán den rezervace v měsíci 1-31
        const s_mesic = document.getElementById(this.id_span_cas_r[2]); // span, kde má být zapsán měsíc rezervace slovně
        const s_rok = document.getElementById(this.id_span_cas_r[3]); // span, kde má být zapsaán rok rezervace
        const s_cas = document.getElementById(this.id_span_cas_r[4]); // span, kde má být zapsán slovně čas rezervace
        const dny = boss.dny; // dny v týdnu splovně Neděle-Pondělí
        const mesice = boss.mesice; // měsíce v roce slovně Leden-Prosinec
        const casy = cas_rezervace.casy; // časy rezervací slovně (1-14)
    ...
    inicializace() {
        ...
        if (location.search) {
            ...
            const sendTextToServer = async (search) => {
                try {
                    ...
                    if (response.ok) {
                        ...
                        const zaznam_pro_odstraneni = result.message; // odpověď, záznam, který má být odstraněn: [(int)$entry['rok'],(int)$entry['mesic'],(int)$entry['den'],(int)$entry['cas_rezervace'],$entry['encrypted_token']] = pole s rokem, mesíce 0-11, dne, číslo času 1-14 a zakódovaný token rezervace
